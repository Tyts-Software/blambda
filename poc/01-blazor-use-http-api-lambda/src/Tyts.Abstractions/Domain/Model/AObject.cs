    ...
    public abstract class AObject : IObject
    {
        ...
        public override int GetHashCode() {
            ...
            unchecked {
                ...
                _cachedHashcode = GetType().GetHashCode();

                if (!IsEmpty(signatureProperties))
                {
                    foreach (var property in signatureProperties)
                    {
                        ...
                        if (value != null)
                            _cachedHashcode = (_cachedHashcode * HASH_MULTIPLIER) ^ value.GetHashCode();
                    }
        ...
        /// </summary>
        public virtual bool HasSameObjectSignatureAs(AObject compareTo) {
            var signatureProperties = GetSignatureProperties();
            ...
            return !IsEmpty(signatureProperties) || base.Equals(compareTo);
        }
        ...
        public virtual IEnumerable<PropertyInfo> GetSignatureProperties() 
        {
            ...
            // http://blogs.msdn.com/jfoscoding/archive/2006/07/18/670497.aspx ->> http://jfoscoding.blogspot.com/
            if (_signaturePropertiesDictionary == null)
                _signaturePropertiesDictionary = new Dictionary<Type, IEnumerable<PropertyInfo>>();

            if (_signaturePropertiesDictionary.TryGetValue(GetType(), out IEnumerable<PropertyInfo> properties))
                return properties;
        ...
        /// !!! the BaseObject already takes care of performance caching, so this method 
